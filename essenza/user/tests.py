...
class RegisterViewTests(TestCase):
    ...
    def test_successful_registration_redirects_and_creates_user(self):
        ...
        self.assertTrue(new_user.check_password(data['password1'])) # La contraseña está hasheada


    #3. Registro con email duplicado muestra error
    def test_registration_with_duplicate_email_shows_error(self):
        User.objects.create_user(username='test', email=self.valid_data['email'], password='test') # Usuario previo creado con mismo email
        data = self.valid_data.copy()
        resp = self.client.post(self.register_url, data) #Intento de registro con el mismo email
        
        self.assertEqual(User.objects.count(), self.initial_user_count + 1) #Solo se añade el usuario creado antes
